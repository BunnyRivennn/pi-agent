...
| `defaultTools` 设置（v0.84.2+） | 在 settings.json 配置**初始**启用的内置工具；扩展/SDK 自定义工具始终保留 | [B02-settings 场景](B02-settings.md) |
| 内置工具名（共 8 个） | `read`、`bash`、`powershell`(v0.84.3，仅 Windows)、`edit`、`write`、`grep`、`find`、`ls` | [sdk_doc/06-tools.md](../sdk_doc/06-tools.md) |
...
## 用 `defaultTools` 配置初始工具集（v0.84.2+，声明式）

`tools` 是**代码里的硬白名单**（覆盖所有工具来源）；如果只想改「启动时默认激活哪些内置工具」、同时保留扩展和 SDK 自定义工具，用 settings 里的 `defaultTools` 更合适：

```jsonc
// ~/.pi/agent/settings.json（全局）或 <cwd>/.pi/settings.json（项目级）
{
  "defaultTools": ["read", "bash", "edit", "write"]
}
```

- 可选值即 8 个内置工具名；省略该项走 pi 标准默认（read/bash/edit/write）
- `[]` 空数组 = 启动时不激活任何内置工具，**但扩展与 SDK customTools 不受影响**——这是它与 `tools: []` / `noTools: "all"` 的关键区别
- 项目级 `defaultTools` 会整体替换全局数组（不是合并）
- 仅决定**初始**激活集，运行时仍可用 `session.setActiveToolsByName()` 调整
- CLI 标志优先级更高：`--tools` 是全来源严格白名单，`--no-tools` / `--no-builtin-tools` / `--exclude-tools` 会覆盖此设置
