...
| `SessionManager.inMemory(cwd?, options?, entries?)` | 创建纯内存会话（测试 / 瞬时场景）；v0.85.0 起第三参可注入外部存储条目恢复历史会话 | [sdk_doc/12-session-manager.md](../sdk_doc/12-session-manager.md) |
...
- **inMemory 也算 session**：内部只是 `persist=false`，仍可正常 `prompt / subscribe / steer`；默认关闭后数据丢失，且 `session.sessionFile === undefined`。
- **⭐ v0.85.0：inMemory 可从外部存储恢复（Web 多租户关键）**：`SessionManager.inMemory(cwd, { id }, entries)` 第三参接受 `FileEntry[]`。用法是运行期只用内存、用 `getEntries()` 自行序列化落库（DB/对象存储，按你自己的用户体系隔离），下次续聊时 `parseSessionEntries(raw)` 还原传入。这样既绕开了 CLI 单用户的 `~/.pi/agent/sessions/` 落盘设计，又具备断点续聊能力。完整示例见 [sdk_doc/12](../sdk_doc/12-session-manager.md#sessionmanagerinmemorycwd-options-entries)。
