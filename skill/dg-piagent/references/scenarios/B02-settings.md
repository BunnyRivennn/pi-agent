...
> **没有专用 setter 的设置项（如 `defaultTools`，v0.84.2+）**：直接写 settings.json，或运行时叠加：
> `sm.applyOverrides({ defaultTools: ["read", "bash", "edit", "write"] })`（不持久化，见下文「运行时叠加」）。该设置决定启动时默认激活哪些**内置**工具，`[]` 表示不激活任何内置工具但保留扩展/SDK 工具；与 `createAgentSession({ tools })` 的硬白名单区别见 [A04](A04-tool-whitelist.md)。
