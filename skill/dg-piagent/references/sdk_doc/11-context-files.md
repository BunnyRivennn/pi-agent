...
- **`AGENTS.override.md`（v0.84.0+）**：某目录若存在 `AGENTS.override.md`，则加载它来**替换同目录的** `AGENTS.md` / `CLAUDE.md`（同目录只取 override 这一个）；**其他目录的上下文文件仍正常分层叠加**。典型用途：在不改动（或无权改动）原有 `AGENTS.md` 的情况下，为某个子树/项目整体替换指令。这是文件发现层的机制，对 `DefaultResourceLoader` 自动发现生效，虚拟 `agentsFilesOverride` 不受影响。
