...
`@earendil-works/pi-coding-agent` 导出完整工具系统，支持使用内置工具（read / bash / powershell / edit / write / grep / find / ls）和注册自定义工具。
...
| `createAllToolDefinitions(cwd, options?)` | `Record<ToolName, ToolDefinition>` | 全部 8 个工具的原始定义，以 Record 形式返回（`powershell` 为 v0.84.3 新增，Windows 原生 shell） |
| `createAllTools(cwd, options?)` | `Record<ToolName, Tool>` | 全部 8 个工具的 AgentTool 实例，以 Record 形式返回 |
