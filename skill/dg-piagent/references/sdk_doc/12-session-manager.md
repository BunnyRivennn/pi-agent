...
- `entries?`: ⭐ **v0.85.0 新增**——注入外部存储的会话条目（`FileEntry[] = SessionHeader | SessionEntry`），用于恢复一个由你自己持久化（如数据库）的历史会话。配合 `getEntries()` / `parseSessionEntries()` 可实现「内存运行 + 外部落库」的完整闭环
...
**从外部存储恢复会话（v0.85.0+，Web 多用户推荐）**：

```ts
// 1. 从数据库取出此前保存的原始 JSONL 内容并解析
import { parseSessionEntries, type FileEntry } from "@earendil-works/pi-coding-agent";
const raw = await db.getSessionJsonl(sessionId);
const entries: FileEntry[] = parseSessionEntries(raw);

// 2. 用 entries 恢复内存会话，后续对话在历史基础上继续
const sm = SessionManager.inMemory(cwd, { id: sessionId }, entries);

// 3. 对话过程中/结束后自行落库（内存模式不会写盘）
//    sm.getEntries() 拿到全部条目（不含 header），自行序列化存储
```

> 等价于文件模式的 `SessionManager.open(path)`，但会话载体在你手里（DB/对象存储）。这让 F01 推荐的 `SessionManager.inMemory()` 多租户方案同时具备了「断点续聊」能力。
...
1. **inMemory 会话默认不持久化**：进程退出数据丢失。v0.85.0 起可通过第三参 `entries` 注入外部存储的条目来恢复历史会话（见上节），但运行期仍不会自动写盘，需要你自行用 `getEntries()` 落库
