...
- `registerMarkdownTransformer`（v0.84.0+）在用户/助手 Markdown 渲染到交互式 transcript 前做**纯展示层**变换（不改 LLM 看到的消息）。签名：`(markdown: string, context: { messageType: "user" | "assistant" | "assistant-thinking"; isStreaming: boolean; availableWidth: number }) => string`，按注册顺序**链式**执行（多个 transformer 依次喂给下一个）。可利用 `messageType` 只改助手消息、`isStreaming` 只在终态做重变换。典型用途：链接卡片展开、术语替换、按宽度裁剪
...
pi.sendUserMessage(
  ...
  options?: {
    deliverAs?: "steer" | "followUp";          // 流式中如何入队
    expandPromptTemplates?: boolean;           // v0.84.2+：是否派发扩展命令并展开 skill 命令/prompt 模板
  }
): void
```
...
> **`expandPromptTemplates` 默认值两层不同**（注意区分）：扩展层 `pi.sendUserMessage()` 默认 **false**（原样发送，v0.84.2 才提供开关）；而外部 `session.prompt(text, { expandPromptTemplates })` 默认 **true**。也就是说从扩展里用 `pi.sendUserMessage("/review")` 想触发命令/模板展开时，必须显式传 `{ expandPromptTemplates: true }`，否则 `/review` 会被当作普通文本。
