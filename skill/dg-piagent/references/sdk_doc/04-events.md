...
| `session_compact_failed` ⭐ v0.84.3（扩展独有） | 压缩失败或被中止 | 否 | 监控压缩异常：payload 含 `reason`（manual/threshold/overflow）/ `aborted` / `errorMessage` / `willRetry` / `fromExtension` |
...
| `ui_prompt_start` ⭐ v0.84.4（扩展独有） | 扩展通过 `ctx.ui` 弹出阻塞式交互（select/confirm/input/editor/custom）开始等待时 | 否 | 区分「agent 工作中」与「等待用户操作 UI」，payload 含 `kind` / 可选 `title` |
| `ui_prompt_end` ⭐ v0.84.4（扩展独有） | 上述 UI 交互结束 | 否 | 交互耗时统计、恢复工作态指示 |
...
> ⚠️ **v0.84.0 起 JSON/RPC 模式（`pi --mode json` / `--mode rpc`）的 `message_update` 变为 delta-only**：不再携带顶层累积 `message` 字段，也不再有 `assistantMessageEvent.partial`（避免输出二次方增长）；改为在顶层给一个常量大小的 `usage`（最新累积 token 用量，provider 不报时为 0）。需要实时文本必须用 `contentIndex` + `delta` 在 `message_start`/`message_end` 之间自行拼装，最终值以 `message_end.message` 为准（仍权威完整）。**进程内 SDK（`session.subscribe` 与扩展层 `pi.on`）不受影响**——事件仍带累积 `message`（已核实 v0.85.1 的 `pi-agent-core/types.d.ts`），本小节描述的 12 子事件协议照常适用。受影响的是 spawn `pi --mode json` 子进程并逐行解析 JSONL 的集成（如 [21-multi-agent](21-multi-agent.md) 的 subagent 模式）。
...
| 事件 | pi.on | subscribe |
|---|:---:|:---:|
| `context` / `tool_call` / `tool_result` / `before_agent_start` / `input` / `model_select` / `thinking_level_select` | ✓ | ✗ 扩展独有（最高频的 7 个） |
| 其他扩展独有：`session_start`/`session_shutdown`/`session_before_*`/`session_compact`/`session_compact_failed`(v0.84.3) / `ui_prompt_start`/`ui_prompt_end`(v0.84.4) / `project_trust` / `resources_discover` / `user_bash` | ✓ | ✗ |
| `agent_start` / `agent_end` / `agent_settled` / `message_*` / `turn_*` / `tool_execution_*` / `session_info_changed` | ✓ | ✓ 两层都有（payload 字段有差异，见下） |
| `queue_update` / `compaction_start`/`compaction_end` / `auto_retry_*` / `thinking_level_changed` / `summarization_retry_*` / `entry_appended` / `bash_execution_update` 等 | ✗ | ✓ subscribe 独有 |

> 注：压缩成功走 subscribe 独有的 `compaction_start`/`compaction_end`；扩展层的 `session_compact` 与 v0.84.3 新增的 `session_compact_failed` 只在 `pi.on` 派发。
