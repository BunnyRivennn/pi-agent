...
## v0.84.0 认证/Provider 相关变更补记

相对 v0.83.0，以下签名有破坏性调整（本文正文描述已按 v0.85.1）：

| 项 | v0.83.0 | v0.84.0+（v0.85.1 基线） |
|----|---------|--------------------------|
| `modelRuntime.refresh()` / `ModelRegistry.refresh()` | 丢弃取消状态与 provider 错误 | 接受 `ModelsRefreshOptions`（`{ allowNetwork?, providers?: string[], force?, signal? }`），返回 `ModelsRefreshResult`（`{ aborted: boolean; errors: ReadonlyMap<string, Error> }`）——调用方可知道哪些 provider 刷新失败 |
| `getApiKeyAndHeaders()` 返回的 headers | `Record<string, string>` | `ProviderHeaders`（值为 `string \| null`）；`null` 是「删除该 header」的标记，转发给 pi-ai stream 时要原样透传，不能把 `null` 字符串化 |
| `setRuntimeApiKey(id, key, options?)` 的 options | catalog 刷新选项 | 改为 `AuthOperationOptions`（认证操作/取消语义）；需要远端刷新时另行 `refresh({ providers: [id], signal })` |
| pi-ai 请求头变换接口 | `ModelsStreamTransforms` | 更名 **`ModelsRequestTransforms`**（`transformHeaders` 现在作用于所有带认证的请求，不再只限于 stream）；影响自定义 streamSimple / Provider 实现里的类型 import |
| 手写原生 `Provider.refreshModels(ctx)` | 直接读写 `ctx.store` | 只读快照 `ctx.stored` + 带 generation 检查的 `ctx.publish({ update, persist? })` 事务。**用 `createProvider({ fetchModels })`（本文 16 文档推荐路径）无需迁移**——恢复/持久化/发布都由 `createProvider` 包掉 |
| OAuth 扩展的 `refreshToken(creds, signal)` | signal 可选 | 必须接受并遵守传入的 `AbortSignal` |
| Google 思考级别类型 | `GoogleThinkingLevel` | `GoogleApiThinkingLevel`（新增规范化用的 `ResolvedGoogleThinkingLevel`） |
